...
/**
 * REFERENCE — academicSearchAgent (given by the assignment).
 *
 * Kept verbatim in intent, refactored to import the shared handleStream
 * per section 0.1. Section 1.3 audit: the original file sorts ascending
 * (`a.similarity - b.similarity`), which keeps the LEAST similar docs.
 * See NOTES-audit.md — flagged, not silently changed here.
 */
...
const basicAcademicSearchResponsePrompt = `
    ...
    If you think there's nothing relevant in the search results, you can say 'Hmm, sorry I could not find any relevant information on this topic. Would you like me to search again or ask something else?'.
...
const createBasicAcademicSearchAnsweringChain = (
  ...
  const retriever = createBasicAcademicSearchRetrieverChain(llm);
  ...
  const rerankDocs = async ({
    ...
    // Descending: keep MOST similar (fixed vs. reference — see NOTES-audit.md)
    return similarity
